...
{
    private readonly IAsyncSession _connection;

    public Repository(IAsyncSession connection)
    {
    ...
    {
        var query = $"MATCH (n:{QueryableEntity.TableName<T>()}) RETURN n";
        var entities = new List<T>();

        var result = _connection.RunAsync(query).Result;
        result.ForEachAsync(record =>
        {
            ...
            entity.FillValues(node.Properties);
            entities.Add(entity);
        }).Wait();
